...
public class VpcVirtualNetworkApplianceManagerImpl extends VirtualNetworkApplianceManagerImpl implements VpcVirtualNetworkApplianceManager {
    ...
    public boolean addVpcRouterToGuestNetwork(final VirtualRouter router, final Network network, final Map<VirtualMachineProfile.Param, Object> params)
        ...
        // HW offload guest networks use PCI passthrough (hostdev VF) for the VR. PCI
        // hotplug of VFs into a running VR does not reliably materialise an eth device
        // inside the guest, so DHCP/SetupGuestNetwork fails with "Unable to apply dhcp
        // entry on router" on the first VM deploy to a new tier. Detect this case and
        // trigger restartVpc(cleanup=true) — the restart path rebuilds the VR with all
        // tiers' hostdev VFs attached at boot via finalizeCommandsOnStart, giving a
        // clean, predictable pairing of eth<N> ↔ tier NIC.
