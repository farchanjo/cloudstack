...
| B5 | `requested-chassis` validation (§5.4) | in_progress | Read-only `OvnChassisLookup` port/OVN adapter and non-matching destination rejection are implemented in the current worktree | commit evidence and Aragog integration validation pending | no auto-pin; preserve operator policy |
