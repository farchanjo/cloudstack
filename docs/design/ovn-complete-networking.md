...
| Isolated-B standalone L3 | ✅ code + control-plane validated; ⚠️ live egress env-blocked | Phase B (§9, commit `faed26a60c`): per-network LR + gateway + source-NAT + public egress + ingress firewall for a non-VPC Isolated OVN network. Strictly additive, `VPC_REGRESSION=NONE` (adversarially verified). NB/northd/SB/ovn-trace all confirm a **complete, correct** L3 datapath identical to a working VPC tier (LR + gw LRP 10.94.1.1 + snat→.41 + dnat .42↔VM + 0/0→.89.1 + DHCP router + firewall ACLs; `ls_in_arp_rsp` proven firing). Destroy path clean (0 orphans, processor drains the 5 new Kinds). **Live egress NOT demonstrated:** the VM's OVS port never binds into the OVN pipeline on the hypervisor (gateway ARP unanswered) — the CloudStack VIF-plugging / mlx5-offload binding layer, **orthogonal to Phase B** (which only writes NB objects, all correct). Deployed to all 3 mgmt nodes; test artifacts torn down clean. |
...
**Phase B — standalone L3 (per-network LR + gateway + source-NAT + public egress) — ✅ CODE COMPLETE + control-plane live-validated; ⚠️ live end-to-end egress blocked by an environmental VM-port-binding issue (NOT a Phase B defect)** (commit `faed26a60c`):
...
- **⚠️ Live end-to-end egress NOT demonstrated — root cause is environmental, not Phase B.** The test VM sits ARPing its gateway (`who-has 10.94.1.1`) with no reply, because its **OVS port never binds into the OVN pipeline on the hypervisor** — the VM's packets don't get the logical-switch datapath metadata, so the (provably-correct) `ls_in_arp_rsp` responder never processes them. This is the CloudStack **VIF-plugging / mlx5-hardware-offload port-binding layer** (the OVS `external_ids:iface-id` does **not** match the LSP name for EITHER this VM or an established working VPC VM — binding here is non-standard), which Phase B does not touch (Phase B only writes NB objects, all correct). Not resolvable non-invasively (`ovn-controller recompute` + correcting `iface-id` to the LSP name did not help; a production `ovn-controller` restart was not attempted unilaterally). Also, no positive control was available (the reference VPC VM was idle/silent, not proven egressing), so it is possible fresh OVN VMs in this environment generally fail to bind. **Follow-up (environmental, out of Phase B scope): investigate the CloudStack KVM-agent OVN VIF driver / ovn-controller port binding on the hypervisor** so that a freshly-deployed OVN VM's OVS port binds to its LSP.
