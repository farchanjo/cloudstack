...
### OVN guest VMs → internal DNS resolvers + working egress (2026-07-05) — DONE + end-to-end validated

Goal: OVN guest VMs receive the fabric's internal resolvers (`217.179.88.54`/`.55`, pdns-recursor cluster) via native DHCP AND actually egress + resolve. Chain of fixes (all deployed to all 3 mgmt nodes, jar-direct):
- **`49ff9c693f` — OvnDhcpService zone-DNS fallback.** OVN DHCP emitted `dns_server` only from `network.getDns1()/getDns2()`, but CloudStack does NOT persist the inherited zone DNS onto the guest network row (it is synthesized at API-read time only), so `network.getDns1()` is null → no `dns_server` at all. Injected `DataCenterDao`; fall back to the zone guest DNS (v4+v6) when the network's is blank. + `cmk update zone dns1=217.179.88.54 dns2=217.179.88.55` (was a DEAD resolver `64.34.88.236`).
- **ConfigDrive `network_data.json={}` fleet-drift — re-synced.** `ConfigDriveNetworkElement` (commit `2021406bc4`: gate network_data on `areServicesSupportedInNetwork` not `isProviderSupportServiceInNetwork(getProvider())`, so SDN/OVN NICs emit it) was STALE on 2 of 3 mgmt nodes → intermittent VMs booting with no NIC. Re-deployed to the stale nodes; all 3 consistent. (A per-class md5 audit across the 3 nodes also found `OvnBgpRedistributeManager` drifted on voldemort → synced. **Lesson: the 3 mgmt fat jars DRIFT across deploy sessions — audit per-class md5 across nodes when behaviour is node-dependent.**)
- **`4d2bc57d01` — firewall baseline must allow egress to the router-peer port.** The Phase A secure-by-default `to-lport (ip4||ip6) drop` is UNQUALIFIED by outport, so it also fired when the LS delivered a VM's egress packet to its router-peer port `rsp-<netuuid>` → ALL egress black-holed on a firewall-enabled network (proven by a clean `ct.new` ovn-trace: fresh egress flow dropped at `ls_out_acl` for `outport==rsp`; the `from-lport` allow only authorizes LS *entry*, not the egress-pipeline delivery to the router). `installDefaultDenyBaseline` now also installs a `to-lport allow-related` for `outport == "rsp-<netuuid>"` (prio 150). Egress *filtering* stays at from-lport; ingress-to-VM stays default-deny (its outport is the VM LSP).
- **End-to-end validated:** fresh useOvn isolated network + VM → NB ACL `to-lport 150 (outport=="rsp-<uuid>") allow-related` present; in-VM `curl https://cloudflare.com` = **HTTP 301** (real egress round-trip), `dig @217.179.88.54 google.com` + `resolvectl query google.com` resolve via the internal resolver, `getent hosts archive.ubuntu.com` resolves. (Minor guest-OS quirk: bare `dig google.com` via the systemd-resolved `127.0.0.53` stub times out on the Ubuntu 24.04 image — EDNS-over-loopback, not network/OVN.) Test artifacts torn down clean.
