...
| Isolated-B standalone L3 | ✅ done + live-validated end-to-end (binding + ingress + egress datapath); internet round-trip gated by env public-block routing | Phase B (§9, commit `faed26a60c`): per-network LR + gateway + source-NAT + public egress + ingress firewall for a non-VPC Isolated OVN network. Strictly additive, `VPC_REGRESSION=NONE` (adversarially verified). Validated live on `ovn-iso-pb2` (VM `10.94.1.31`): VM **binds** (iface-id=`lsp-<nicuuid>`, ovn-installed=true), resolves its gateway, gets L2/L3 + default route; **ingress round-trips** (Mac→`.89.42` ping 3/3 + SSH open, ingress firewall tcp/22+icmp enforced); **egress datapath confirmed** (VM traffic SNAT'd to `.89.42` and leaves the physical uplink `dx6p0` to the internet — tcpdump at every hop). Destroy path clean (0 orphans, processor drains the 5 new Kinds). **The binding "blocker" seen on the first test was a MISSING `useOvn` OFFERING TAG** (required by `HypervisorGuruBase` to set `NicTO.useOvn` → agent uses `OvnVifDriver` → stamps `iface-id=lsp-<nicuuid>`); a test-setup omission, not a code defect — all working OVN offerings carry `tags=useOvn`. **Internet round-trip** for VM-initiated egress is gated by an **environmental fabric-edge matter** (not the plugin): the CloudStack public block `217.179.89.0/24` is not internet-return-routable via the site's real internet uplink (`64.34.88.225`) — proven by `ping -I 217.179.89.2 8.8.8.8` = 100% loss from the gateway chassis itself while default-source (`64.34.88.227`) = 3/3; identical to the mode-4/P4 deferred public-announce, and it affects all CloudStack public egress. Deployed to all 3 mgmt nodes; test artifacts torn down clean. |
...
- **Internet round-trip (VM-initiated egress reply) is gated by an environmental fabric-edge matter, not the plugin.** The CloudStack public block `217.179.89.0/24` (vlan 2988, the only public range) is **not internet-return-routable** via the site's real internet uplink `64.34.88.225` — proven from the gateway chassis itself: `ping 8.8.8.8` (default src `64.34.88.227`) = 3/3, but `ping -I 217.179.89.2 8.8.8.8` = 100% loss. So a VM SNAT'd to `.89.x` egresses correctly but its reply never returns. This is the same deferred outward-facing public-block routing noted for mode-4/P4 and affects **all** CloudStack public egress (VPC included). **Follow-up (environmental, out of Phase B scope): make `217.179.89.0/24` internet-return-routable (upstream announce/route) or add an edge NAT `.89.x → 64.34.88.x`, so any CloudStack public egress round-trips.**
